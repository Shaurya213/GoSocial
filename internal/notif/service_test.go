...
// var mockDeviceRepo *MockDeviceRepository
...
// func (m *MockNotificationRepository) ByID(ctx context.Context, id uint) (interface{}, error) {
// 	args := m.Called(ctx, id)
// 	return args.Get(0), args.Error(1)
// }

// // FIXED: userID should be uint, not string
// func (m *MockNotificationRepository) ByUserID(ctx context.Context, userID uint, limit, offset int) ([]interface{}, error) {
// 	args := m.Called(ctx, userID, limit, offset)
// 	return args.Get(0).([]interface{}), args.Error(1)
// }
...
// // FIXED: id should be uint, not string
// func (m *MockNotificationRepository) UpdateStatus(ctx context.Context, id uint, status string) error {
// 	args := m.Called(ctx, id, status)
// 	return args.Error(0)
// }

// // FIXED: both parameters should be uint, not string
// func (m *MockNotificationRepository) MarkAsRead(ctx context.Context, notificationID uint, userID uint) error {
...
// // FIXED: userID should be uint64, not string
...
// // MockEmailService and MockTestObserver remain the same
// type MockEmailService struct {
// 	mock.Mock
// }
...
// var mockRepo *MockNotificationRepository
...
func (m *MockNotificationRepository) UnreadCount(ctx context.Context, userID uint) (int64, error) {
	args := m.Called(ctx, userID)
	return args.Get(0).(int64), args.Error(1)
}
...
// Test examples with correct type usage
...
func TestNotificationService_SendNotification_Valid(t *testing.T) {
	mockRepo := &MockNotificationRepository{}
	mockDeviceRepo := &MockDeviceRepository{}
	service := NewNotificationService(&config.Config{}, mockRepo, mockDeviceRepo, nil, nil)
	defer service.Shutdown()
	...
	assert.NoError(t, service.SendNotification(context.Background(), event))
}
...
func TestNotificationService_MarkAsRead(t *testing.T) {
	...
	service := NewNotificationService(&config.Config{}, mockRepo, mockDeviceRepo, nil, nil)
	defer service.Shutdown()
	mockRepo.On("MarkAsRead", mock.Anything, uint(1), uint(2)).Return(nil)
	assert.NoError(t, service.MarkAsRead(context.Background(), 1, 2))
	mockRepo.AssertExpectations(t)
}

func TestDeviceRepoInterfaceConformance(t *testing.T) {
	mockDeviceRepo := &MockDeviceRepository{}
	mockDeviceRepo.On("ActiveByUserID", mock.Anything, uint64(1)).Return([]interface{}{}, nil)
	_, err := mockDeviceRepo.ActiveByUserID(context.Background(), uint64(1))
	assert.NoError(t, err)
}

func TestNotificationRepoInterfaceConformance(t *testing.T) {
	mockRepo := &MockNotificationRepository{}
	mockRepo.On("Delete", mock.Anything, uint(1)).Return(nil)
	assert.NoError(t, mockRepo.Delete(context.Background(), 1))
}
